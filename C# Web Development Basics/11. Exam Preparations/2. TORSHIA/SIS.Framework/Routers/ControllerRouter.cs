    ...
    public class ControllerRouter : IMvcRouter
    {
        ...
        private Controller GetController(string controllerName, IHttpRequest request)
        {
            if (controllerName != null)
            {
                ...
                var controllerType = Type.GetType(controllerTypeFullName);
                var controllerInstance = (Controller)this.dependencyContainer.CreateInstance(controllerType);
        ...
        private string[] ExtractControllerAndActionNames(IHttpRequest request)
        {
            ...
            {
                string[] requestUrlSplit = request.Path.Split(
        ...
        {
            foreach (MethodInfo controllerAction in this.GetSuitableMethods(controller, actionName))
            {
                ...
                if (!controllerActionAttributes.Any()
                    && requestMethod.ToUpper() == DefaultControllerActionRequestMethod)
                {
                    return controllerAction;
                }

                foreach (HttpMethodAttribute attribute in controllerActionAttributes)
                {
        ...
        private object[] MapActionParameters(Controller controller, MethodInfo action, IHttpRequest httpRequest)
        {
            ...
            for (int index = 0; index < actionParametersInfo.Length; index++)
            {
                ...
                {
                    object bindingModel = this.ProcessBindingModelParameter(currentParameterInfo, httpRequest);
